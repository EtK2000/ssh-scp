...
class KeySupplierGithub implements KeySupplier {
	...
	private final String clientID, clientSecret, owner, repo;
	...
	public void write(JsonWriter jw) throws IOException {
		jw.name("github").beginObject();
		{
			jw.name("client_id").value(clientID);
			jw.name("client_secret").value(clientSecret);
			jw.name("repo_name").value(repo);
			jw.name("repo_owner").value(owner);
		}
		jw.endObject();
	}
}